function  [G_Zyl,Pl] = MUSICgenG2(X,Y,Z,Phi,xl,yl,zl)
...
% Diese Funktion liefert als resultat alle komponenten des Feldes, sowie die
...
%     G_Zyl = NaN([length(Pl) length(P) 3]);
    ...
    for j=1:length(Pl)
        for i = 1:length(P)
            ...
            [G_Phi,G_R,G_Z] = cart2polVect(G(1,:),G(2,:),G(3,:),PPhi(i,1));
%             G = mu_0/(4*pi*norm(r)^2) * (3*r*(mu'*r)-mu*norm(r)^2)/(norm(r)^3);
%             [~,G_R,~] = cart2polVect(G(1,:),G(2,:),G(3,:),PPhi(i,1));
%             G_Zyl(j,i,:) = G_R;
            G_Zyl(i,(j-1)*3+1:(j-1)*3+3) = G_R;
            G_Zyl(i+length(P),(j-1)*3+1:(j-1)*3+3) = G_Phi;
            G_Zyl(i+length(P)*2,(j-1)*3+1:(j-1)*3+3) = G_Z;
        end
