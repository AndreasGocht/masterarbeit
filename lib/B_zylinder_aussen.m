function [ B_1,B_2,B_3,B_4 ] = B_zylinder_aussen(nu,kappa_z,d_kappa_z,l,R,phi,z)
% B_zylinder_aussen gibt die ergebinsse der verschiedenen
...
if (kappa_z~=0)
    if(nu~=0)
        dZdR = w.*kappa_z.*(besseli(nu-1,w.*kappa_z.*R)+besseli(nu+1,w.*kappa_z.*R))./2;
    else
        dZdR = w.*kappa_z.*besseli(1,w.*kappa_z.*R);
    end
    Z = besseli(nu,w.*kappa_z.*R);
else
   if(nu~=0)
       dZdR = nu.*R.^(nu-1);
       Z = R.^nu;
...
if(nu~=0)
    ...
    B_1(3,:) = + mu_0 .* d_kappa_z .* Z .* kappa_z.*sin(w.*kappa_z.*z);

    B_2(1,:) = - mu_0 .* d_kappa_z .* dZdR .*sin(w.*kappa_z.*z);
    B_2(2,:) = - mu_0 .* d_kappa_z .* 1./R ;
    B_2(3,:) = - mu_0 .* d_kappa_z .* Z .* kappa_z.*cos(w.*kappa_z.*z);
